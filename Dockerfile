...
RUN curl -s https://repo.jellyfin.org/jellyfin_team.gpg.key | apt-key add - && \
  ...
  apt-get update  &&\
  apt-get -y install --no-install-recommends jellyfin-server=20210913.13-unstable jellyfin-web=${BUILD_TAG} jellyfin-ffmpeg mesa-va-drivers jellyfin-ffmpeg openssl locales && \
